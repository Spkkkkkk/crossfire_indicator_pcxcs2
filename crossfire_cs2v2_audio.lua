...
local function play_kill_sound()
    ...
    if does_file_exist(sound_path) then
        snd = load_sound(sound_path)
        local ok = play_sound(snd, 0.1)
        log(string.format("[kill-photo] play_sound(%s) -> %s", sound_path, tostring(ok)))
    else
...
function main()
    ...
    if winapi and winapi.play_sound and does_file_exist(asset_path("1.wav")) then
        local ok = winapi.play_sound(asset_path("1.wav"))
        log(string.format("[kill-photo] Test audio inicial 1.wav -> %s", tostring(ok)))
    else
    ...
    log("[kill-photo] Version con audio cargada.")
    return 1
end
...
function on_unload()
    ...
    log("[kill-photo] Version con audio descargada.")
end
